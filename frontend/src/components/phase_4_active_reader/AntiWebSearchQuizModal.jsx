  ...
  const handleSubmitQuiz = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        // Sync quiz attempt to localStorage for the student records dashboard (both user-scoped and global)
        try {
          const userKey = studentId || 'default';
          const prevScoped = JSON.parse(localStorage.getItem(`study_prep_quizzes_${userKey}`) || '[]');
          const prevGlobal = JSON.parse(localStorage.getItem('study_prep_quizzes') || '[]');
          const newQuizEntry = {
            id: data.attempt_id || Date.now(),
          ...
          localStorage.setItem(`study_prep_quizzes_${userKey}`, JSON.stringify([newQuizEntry, ...prevScoped]));
          localStorage.setItem('study_prep_quizzes', JSON.stringify([newQuizEntry, ...prevGlobal]));
        } catch (e) {}
      }
    } catch (err) {
