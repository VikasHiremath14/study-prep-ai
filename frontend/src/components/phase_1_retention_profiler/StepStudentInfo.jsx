...
import { User, GraduationCap, ArrowRight, Sparkles, Cpu, Brain, Rocket, Code2, TrendingUp } from 'lucide-react';
...
const INTERESTS = [
  { id: 'ai_ml', label: 'AI & Neural Networks', icon: Cpu, color: '#6366f1' },
  { id: 'neuroscience', label: 'Brain & Neuroscience', icon: Brain, color: '#10b981' },
  { id: 'astrophysics', label: 'Space & Physics', icon: Rocket, color: '#a855f7' },
  { id: 'computer_science', label: 'Algorithms & Computing', icon: Code2, color: '#06b6d4' },
  { id: 'robotics', label: 'Robotics & Hardware', icon: Sparkles, color: '#f59e0b' },
  { id: 'finance', label: 'Finance & Economics', icon: TrendingUp, color: '#ec4899' }
];
    ...
    <div style={{ maxWidth: '680px', margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: '32px' }}>
        <div className="badge" style={{ marginBottom: '12px' }}>
          <Sparkles size={14} />
          Step 1 of 5 &bull; Student Profile & Interests
        </div>
      ...
      <div className="glass-panel" style={{ padding: '32px' }}>
            ...
            {GRADE_LEVELS.map((g) => {
              ...
              return (
                ...
                >
                  ...
                  <p style={{ fontSize: '0.78rem', color: 'var(--text-dim)', paddingLeft: '28px' }}>
                    {g.desc}
                  </p>
                </div>
              );
        ...
        {/* Primary Study / Research Interest */}
        <div style={{ marginBottom: '32px' }}>
          <label style={{ display: 'block', fontSize: '0.88rem', fontWeight: 600, marginBottom: '12px', color: 'var(--text-main)' }}>
            Primary Technical / Academic Interest
          </label>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '10px' }}>
            {INTERESTS.map((item) => {
              const isSelected = (data.interest || 'ai_ml') === item.id;
              const Icon = item.icon;
              return (
                <div
                  key={item.id}
                  onClick={() => onChange({ ...data, interest: item.id })}
                  style={{
                    padding: '12px',
                    ...
                    gap: '10px',
                    transition: 'all 0.2s ease',
                    boxShadow: isSelected ? `0 0 12px ${item.color}35` : 'none'
                  }}
                >
                  <Icon size={18} color={isSelected ? item.color : 'var(--text-dim)'} />
